...
// returns a promise
const cachePromiseFunction = fn => {
  //create cache to store key/values
  const cache = {}; 
  //return anonymous function
  return function (arg) { 
    //check if url string is a key in cache
    if (cache[arg]){
      //if it is, return the associated value
      return cache[arg];
    } else {
    //otherwise create a key of the fn arg passed into promise.resolve
    //return the value at this key
    cache[arg] = Promise.resolve(fn(arg));
    return cache[arg];
   }
  };
}
